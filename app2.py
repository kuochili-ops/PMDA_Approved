...
def translate_to_en(text):
    ...
    # 關鍵修復：強制對 Headers 進行字串處理，避免非 Latin 字元干擾
    headers = {
...
st.set_page_config(layout="wide", page_title="PMDA 12月修復版")
st.title("💊 PMDA 深度解析系統")

f = st.file_uploader("上傳 PMDA 原始 Excel", type=['xlsx'])

if f:
    xl = pd.ExcelFile(f)
    selected_sheet = st.selectbox("請選擇分頁：", xl.sheet_names, index=len(xl.sheet_names)-1)
    ...
    if st.button("開始解析"):
        ...
        for i in range(min(20, len(df_raw))):
            ...
            if any('販' in x for x in row_vals):
                ...
                for idx, v in enumerate(row_vals):
                    v_c = str(v).replace(' ', '').replace('\n', '')
                    if '販' in v_c: t_col = idx
                    if 'No' in v_c: n_col = idx
                    if '効能' in v_c or '效果' in v_c: ind_col = idx
                    if '承認' in v_c and idx != t_col: status_col = idx
                break
        ...
        if h_idx != -1:
            ...
            st.download_button("📥 下載解析成果", out.getvalue(), f"PMDA_{selected_sheet}_Fixed.xlsx")
        else:
            st.error("欄位定位失敗，請確認 Excel 標題包含『販売名』。")
