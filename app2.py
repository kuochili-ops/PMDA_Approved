...
> 🛠️ **修正重點**：鎖定「欧文商標名」與「欧文一般名」標籤，修正 ID 格式錯誤，補完 Syntax 結構。
""")
st.divider()
...
def fetch_dual_strings(japic_id_input, trade_jp_full):
    ...
    try:
        # 1. JapicID 格式化：確保是 8 位數字且無小數點
        raw_id = str(japic_id_input).split('.')[0].strip()
        final_id = re.sub(r'[^0-9]', '', raw_id)
        
        # 若 ID 長度異常 (正確應為 8 位)，嘗試從商品名搜尋獲取正確 ID
        if not (5 <= len(final_id) <= 9):
            search_url = f"https://www.kegg.jp/medicus-bin/search_drug?search_keyword={quote(trade_jp_full[:10])}"
        ...
        if final_id:
            ...
            if target_span:
                ...
                if parent_p:
                    full_txt = parent_p.get_text(strip=True)
                    # 提取「欧文商標名」之後的內容
...
if f:
    try:
        ...
        if not found_header:
            st.error("❌ 找不到表頭，請確認 Excel 是否包含 '商品名' 或 'JapicID' 欄位。")
        else:
            # 整理待處理清單
            data_list = []
            ...
            if st.button("🚀 開始執行深度對位"):
                ...
                st.download_button(
                    label="📥 下載完整結果 Excel",
                    data=out.getvalue(),
                    file_name=f"PMDA_Approved_Analysis_{VERSION_DATE}.xlsx",
